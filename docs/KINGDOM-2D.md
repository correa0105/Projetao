...
**Edição de objetos (24/09/2026):** arrastar diretamente um objeto nos modos Adicionar ou Selecionar move o sprite; o chão continua arrastável para navegar. Um objeto recém-colocado já fica selecionado e pode ser movido imediatamente. Ctrl + arrastar no chão seleciona por área; Ctrl + clique ajusta o grupo; Alt + clique no modo Adicionar coloca um novo item mesmo sobre um sprite. A seleção pode ser movida com setas no painel ou teclado, em passos configuráveis de pixels; Shift + setas usa cinco passos. Duplicar cria cópias deslocadas e seleciona as cópias; todas as ações usam o desfazer e o salvamento privado já existentes.

**Zoom e enquadramento particular (24/09/2026):** no editor, o zoom absoluto cobre 0,03–32. Depois de ajustar zoom, centro e giro, **Definir visão atual como 100%** persiste essa câmera em `kingdom_editor_views` (migration 014). Ao recarregar o reino, a câmera inicia nesse estado e a UI exibe 100%; fora do editor, o usuário navega entre 100 e 130% relativos à visão salva. **Restaurar visão inicial** exclui essa preferência e recentraliza a câmera. A preferência só se aplica à versão do background para a qual foi salva; upload ou restauração do fundo começa com a visão inicial sem afetar objetos.

**Proporção de fundos enviados (24/09/2026):** imagens particulares do editor aparecem sem compressão vertical (`KingdomViewport.tilt=1`), preservando a proporção de origem mesmo quando retangulares. A antiga inclinação de 0,58 permanece somente no chão padrão ilustrado. Desenho, hit testing, câmera, limites e colocação compartilham esse valor, mantendo os sprites no mesmo local relativo ao fundo. O upload 2048 × 1536 salvo pelo usuário não foi alterado; basta recarregar a visão para vê-lo corrigido.

**Estado atual do editor (24/09/2026):** o chão padrão continua `ground-trails.png` (3072²), mas cada usuário pode enviar um PNG, JPEG ou WebP privado até 128 MB, 12.288 px por lado e 100 MP. A migration 013 armazena a imagem no PostgreSQL; a API autenticada entrega metadados e os bytes somente ao autor. A área virtual usa 15000/3072 unidades por pixel real, ajustando os limites de câmera e colocação a backgrounds grandes e retangulares. Arraste comum navega; Ctrl + clique alterna objetos e Ctrl + arraste marca área ou move o grupo selecionado. Ctrl + Shift + arraste acrescenta à seleção; Selecionar todos, Delete, escala, giro, excluir e desfazer funcionam em grupo. O fundo pode ser restaurado sem apagar objetos. A névoa regional foi inteiramente retirada; o vazio fora da imagem tem cor neutra. Menções à névoa abaixo são históricas.
