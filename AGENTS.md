...
- A **visão do reino** usa por padrão `public/kingdom/ground-coast.webp` (4096 × 2078 px) e `coast-mask.png`: mar à esquerda do contorno enviado e grama oliva à direita, sem objetos automáticos. `src/kingdom-relief.ts` gera uma malha 3D de relevo suave e água em Three.js/WebGL; `src/kingdom-scene.ts` projeta sprites do editor no Canvas 2D usando os oito quadros direcionais e a altura local da malha. O editor salva objetos privados em `kingdom_editor_drafts` e, desde a migration 013, um background privado PNG/JPEG/WebP em `kingdom_editor_backgrounds`. A área virtual é calculada pela resolução do background a 15000/3072 unidades por pixel; câmera, colocação e navegação respeitam seus limites. **Fundos enviados permanecem planos e usam projeção sem inclinação (`tilt=1`) para manter a proporção original; o terreno costeiro padrão usa `tilt=0.58`.** O editor libera zoom absoluto de 0,03 a 32, inclusive por controle deslizante; **Definir visão atual como 100%** salva zoom, centro e giro privados na migration 014, vinculados à versão atual do fundo. Fora do editor, o zoom volta ao intervalo de 100–130% relativo à visão salva; Centralizar restaura a visão de 100%. No editor, arrastar um item nos modos Adicionar ou Selecionar move o item ou grupo; arrastar o chão navega. Ctrl + arrastar no chão seleciona uma área; Ctrl + clique alterna itens. Alt + clique no modo Adicionar permite inserir sobre um item existente. O painel oferece passo de movimento em pixels, setas na interface e no teclado, Shift + setas para cinco passos, duplicação, escala, giro, exclusão e desfazer. **Não há névoa regional**; `src/kingdom-atmosphere.ts` foi removido. Preservar os rascunhos existentes, os seis locais SQL, missões, usuários e o Mundo 3D. Validar com `ATLAS_BROWSER_GPU=1 npm run test:atlas -- --terrain-only` e `--editor-only`, além de `npm test` quando regras ou dados mudarem. Especificação em `docs/KINGDOM-2D.md`. Não apagar `public/atlas-materials`: o Mundo usa essas texturas.
